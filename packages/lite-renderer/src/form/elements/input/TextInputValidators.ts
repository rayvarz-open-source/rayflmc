...
export function makeTextInputValidations(localization: TextInputValidationLocalization): { [name: string]: TextInputValidatorBuilder; } {
    return {
        contains: (compare: string) => (value: string) => new ValidationResult(value.includes(compare), localization.contains.replace("{compare}", compare)),
        notEmpty: () => (value: string) => new ValidationResult(value != '' && value != null, localization.notEmpty),
        maxLength: (length: number) => (value: string) => new ValidationResult(value.length <= length, localization.maxLength.replace("{length}", length.toString())),
        minLength: (length: number) => (value: string) => new ValidationResult(value.length >= length, localization.maxLength.replace("{length}", length.toString())),
    }
}
