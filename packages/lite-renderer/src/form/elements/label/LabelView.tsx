...
export default function LabelView({ element, weight }: Props) {
  ...
  const [noWrap, setNoWrap] = React.useState<NoWrap>(
    () => element.noWrapContainer.value
  );
  const [gutterBottom, setGutterBottom] = React.useState<GutterBottom>(
    () => element.gutterBottomContainer.value
  );
  ...
  const [display, setDisplay] = React.useState<Display>(
    () => element.displayContainer.value
  );
  const [align, setAlign] = React.useState<Align>(
    () => element.alignContainer.value
  );
  ...
  React.useEffect(() => {
    ...
    let noWrapSub = element.noWrapContainer.subscribe({
      next: v => setNoWrap(v)
    });
    let gutterBottomSub = element.gutterBottomContainer.subscribe({
    ...
    let displaySub = element.displayContainer.subscribe({
      next: v => setDisplay(v)
    });
